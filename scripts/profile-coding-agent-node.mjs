...
const agentDirEnvName = "MIDNIGHT_SERVER_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "MIDNIGHT_SERVER_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific MIDNIGHT_SERVER_CODING_AGENT_DIR for the benchmark run
  ...
  --no-offline           Do not force MIDNIGHT_SERVER_OFFLINE=1 / MIDNIGHT_SERVER_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	const env = { ...process.env, MIDNIGHT_SERVER_TIMING: "1" };
	if (options.agentDir) {
	...
	if (options.offline) {
		...
		env.MIDNIGHT_SERVER_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
