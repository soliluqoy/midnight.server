...
| Automatic first-run fetch | `engine-manager.ts`'s `resolveLocalAssets()`: downloads the model, and (Windows x64 only) the engine, the first time either is missing, instead of erroring; an explicit `MIDNIGHT_SERVER_MODEL`/`MIDNIGHT_SERVER_ENGINE_DIR` override that points at nothing still fails closed rather than being routed around | `test/midnight-engine-manager.test.ts` |
