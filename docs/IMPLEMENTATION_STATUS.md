...
| Process ownership, Linux/macOS | `POSIX_HOST` in `engine.ts`: `/bin/sh` wrapper starts `llama-server` in the background, kills it when the CLI's stdin pipe reaches EOF (CLI exit or SIGKILL) or the wrapper gets SIGTERM, and exits with the engine's code | `test/midnight-engine-host.test.ts` (skipped on Windows; runs in `midnight-release.yml`); `scripts/verify-release.mjs --smoke` SIGKILLs the CLI during a real engine run. Wrapper logic checked by hand under Git Bash; **not yet run on Linux or macOS** |
...
| Drift watch two-stage check | `drift-watch.ts`: a gate reads one grammar-constrained status token (`on_track`/`drifting`/`off_task`, each with a distinct first token) with `top_logprobs`, renormalizes over the labels, and decides; only a not-on-track probability at or above `MIDNIGHT_SERVER_DRIFTWATCH_CONFIDENCE` runs the explain call, whose schema fixes the status to the gate's and which reuses the cached system+transcript prefix. No logprobs: falls back to the single full check | `test/suite/midnight-drift-watch.test.ts`, `test/midnight-drift-watch.test.ts`, `test/midnight-engine-payload.test.ts`; real model: distinct first tokens and gate probabilities in `test/midnight-engine.integration.test.ts` |
...
| Linux/macOS packaging | `scripts/build-unix.sh`, `package-unix.sh` (tarball; `.deb` for linux-x64), `verify-release.mjs`, `write-release-manifest.mjs`, `get.sh`; `.github/workflows/midnight-release.yml` builds Windows x64, Linux x64, macOS arm64 and x64 on native runners and attaches them to a draft release | **Not yet run.** macOS builds are ad-hoc signed only |
...
`npm run check` passes. All midnight tests pass, including the real-model integration test. Its helper `inspect` case had passed by chance: at temperature 1 without thinking the helper often answered its question wrong. Helper tasks now decode at temperature 0, `inspect` thinks by default, and the test checks the answer. Three existing tests that assumed Bash as the Windows default tool were updated.
