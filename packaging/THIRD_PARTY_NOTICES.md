...
| llama.cpp / ggml (`llama-server` and libraries in `engine/cpu`, or `engine/metal` on Apple Silicon) | release `b11166`, commit `a72e04abe0fe9b36e203033ac71bd5f379c35bc5` | MIT | `licenses/llama.cpp-LICENSE.txt` |
...
`midnight-host.exe` (Windows) is built from `native/midnight-host/Program.cs` in this repository. On Linux and macOS the engine runs under a `/bin/sh` wrapper defined in `packages/coding-agent/src/midnight/engine.ts`.
