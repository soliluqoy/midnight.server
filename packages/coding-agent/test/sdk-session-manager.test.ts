...
describe("createAgentSession session manager defaults", () => {
	...
	it("exposes current session state to the built-in bash tool", async () => {
		...
		const { session } = await createAgentSession({
			...
			model: model!,
			thinkingLevel: "high",
			// Windows defaults to the powershell tool; this test exercises bash explicitly.
			tools: ["read", "bash", "edit", "write"],
		});
		expect(session.sessionFile).toBeTruthy();
		expect(session.systemPrompt).toContain(
			"You can inspect MIDNIGHT_SERVER_* environment variables for current model and session details.",
		);

		const bashTool = session.agent.state.tools.find((tool) => tool.name === "bash");
		expect(bashTool).toBeTruthy();
		const result = await bashTool!.execute("test", {
			command: `printf '%s\\n' "$MIDNIGHT_SERVER_SESSION_ID" "$MIDNIGHT_SERVER_SESSION_FILE" "$MIDNIGHT_SERVER_PROVIDER" "$MIDNIGHT_SERVER_MODEL" "$MIDNIGHT_SERVER_REASONING_LEVEL"`,
		});
		const output = result.content
			.filter((item): item is { type: "text"; text: string } => item.type === "text")
			.map((item) => item.text)
