...
const originalOffline = process.env.MIDNIGHT_SERVER_OFFLINE;
...
afterEach(() => {
	if (originalOffline === undefined) delete process.env.MIDNIGHT_SERVER_OFFLINE;
	else process.env.MIDNIGHT_SERVER_OFFLINE = originalOffline;
	vi.unstubAllGlobals();
});
