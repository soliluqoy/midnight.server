...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.MIDNIGHT_SERVER_TELEMETRY;
		delete process.env.MIDNIGHT_SERVER_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.MIDNIGHT_SERVER_TELEMETRY = originalTelemetryEnv;
		}
