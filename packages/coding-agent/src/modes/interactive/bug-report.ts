...
import { APP_NAME, BUG_REPORT_URL } from "../../config.ts";
...
/**
 * Run the `/bug` flow: consent, optional summary, then export a zip archive.
 * midnight.server never uploads reports: upstream's upload target is the Pi developers' service.
 */
...
async function exportZip(context: BugReportContext, bundle: BugReportBundle): Promise<void> {
	...
	context.showStatus(
		`Bug report exported to: ${archivePath}\nReport ID: ${bundle.metadata.id}\nAttach it to an issue at ${BUG_REPORT_URL}`,
	);
}
