...
A provider extension connects midnight.server to a model service that needs custom authentication, model discovery, request handling, or streaming. If the service already speaks a supported API, configure it in `models.json` instead.

Provider extensions run inside midnight.server and can inspect credentials, prompts, tool definitions, model responses, and usage. Treat them as trusted code and avoid logging secrets or provider payloads.
...
Call `pi.registerProvider()` from the extension factory. midnight.server waits for asynchronous factories before startup continues, so providers registered there are available to startup model selection and `midnight.server --list-models`.
...
Prefer a complete provider for new integrations that own more than static endpoint and model metadata. midnight.server composes `models.json` overrides above a registered native provider.
...
An OAuth provider supplies a display name, login flow, token refresh, and access-token resolution. After registration it appears in `/login`, and midnight.server stores returned credentials in `~/.midnight.server/agent/auth.json`.
...
Set `promptCache.short` or `promptCache.long` to the provider's best-effort cache lifetime in seconds when midnight.server should keep an idle prompt cache warm. Leave them unset to disable cache warming for that retention tier.
...
- Legacy `ProviderConfig.refreshModels` returns model definitions. midnight.server replaces that registration’s live models with the returned list and applies any requested persistence.
...
Use one of midnight.server AI’s API implementations whenever the provider protocol matches it.
...
This is safer than copying a stream implementation because it preserves midnight.server’s message conversion, tool handling, usage accounting, cancellation, and compatibility behavior.
...
These hooks power extension request inspection and response-header events. Omitting them makes the provider behave differently from midnight.server’s built-in providers.
...
midnight.server can compact and retry after recognized context-overflow errors. If the service uses an unknown message, normalize only that provider’s overflow response to `context_length_exceeded` in a guarded `message_end` handler.

Do not rewrite rate limits or transient provider failures as context overflow. Those failures use midnight.server’s normal retry behavior instead.
...
Run the extension directly while developing, then move it to a discovered extension location or distribute it through a [midnight.server package](packages.md). Use `/reload` after changing a discovered provider extension in an active session.
