...
```
~/.midnight.server/agent/sessions/--<path>--/<timestamp>_<session-id>.jsonl
```

By default, `<session-id>` is a UUID. Callers can supply a custom ID through the SDK or `--session-id`. For `<path>`, midnight.server removes the leading path separator and replaces `/`, `\\`, and `:` with `-`.

## Deleting Sessions

Sessions can be removed by deleting their `.jsonl` files under `~/.midnight.server/agent/sessions/`.

midnight.server also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, midnight.server uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([midnight.server](https://github.com/earendil-works/pi)):
...
Usage entries contribute to session token and cost totals. midnight.server hides them from the conversation tree. Consumers should treat unknown `kind` values as normal usage rather than rejecting them.
...
`firstKeptEntryId` is required. It identifies the first entry retained from before the compaction entry. When rebuilding context, midnight.server replaces older summarized entries with the compaction summary and keeps the range beginning at this entry. A retain-none compaction stores its own ID in this field, so no preceding entries are retained.
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if midnight.server-generated (legacy field name)
