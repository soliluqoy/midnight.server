...
midnight.server uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.midnight.server/agent/settings.json` | Global (all projects) |
| `.midnight.server/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`MIDNIGHT_SERVER_EXPERIMENTAL=1`) |
...
| `tuiMode` | string | `"fullscreen"` | Interactive TUI mode: `"fullscreen"` (transcript, input dock and session sidebar on the alternate screen) or `"regular"` (inline in the terminal scrollback). Changes from `/settings` apply immediately; `--tui-mode` overrides this setting at startup |
...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://pi.dev/api/report-install` and midnight.server attribution headers for OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out disables both. It does not disable update checks; midnight.server can still fetch `https://pi.dev/api/latest-version` to look for the latest version.

Set `MIDNIGHT_SERVER_SKIP_VERSION_CHECK=1` to disable the midnight.server version update check. Use `--offline` or `MIDNIGHT_SERVER_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before midnight.server sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.midnight.server/agent/npm/`; project-scoped npm packages install under `.midnight.server/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, midnight.server uses its standard defaults |
...
```json
{ "sessionDir": ".midnight.server/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `MIDNIGHT_SERVER_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.midnight.server/agent/settings.json` resolve relative to `~/.midnight.server/agent`. Paths in `.midnight.server/settings.json` resolve relative to `.midnight.server`. Absolute paths and `~` are supported.
...
Project settings (`.midnight.server/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.midnight.server/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .midnight.server/settings.json (project)
{
