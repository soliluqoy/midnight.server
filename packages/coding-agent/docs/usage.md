...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want midnight.server to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically to `~/.midnight.server/agent/sessions/`, organized by working directory.

```bash
midnight.server -c                  # Continue most recent session
midnight.server -r                  # Browse and select a session
midnight.server --no-session        # Ephemeral mode; do not save
midnight.server --name "my task"    # Set session display name at startup
midnight.server --session <path|id> # Use a specific session file or session ID
midnight.server --fork <path|id>    # Fork a session into a new session file
```
...
If a directory contains `AGENTS.override.md`, midnight.server loads it instead of `AGENTS.md` or `CLAUDE.md` from that directory. Context files from other directories still layer normally.
...
- `.midnight.server/SYSTEM.md` for a project
- `~/.midnight.server/agent/SYSTEM.md` globally
...
If you use midnight.server for open source work and want to publish sessions for model, prompt, tool, and evaluation research, see [`badlogic/pi-share-hf`](https://github.com/badlogic/pi-share-hf). It publishes sessions to Hugging Face datasets.
...
midnight.server update --all              # Update midnight.server and packages; reconcile pinned git refs
midnight.server update --extensions       # Update packages only; reconcile pinned git refs
...
These commands manage midnight.server packages and `midnight.server update` can update the midnight.server CLI installation. To uninstall midnight.server itself, see [Quickstart](quickstart.md#uninstall). `midnight.server config` and project package commands accept `--approve`/`--no-approve` to trust or ignore project-local settings for one command. `midnight.server update` never prompts for project trust.

See [midnight.server Packages](packages.md) for package sources and security notes.
...
```bash
midnight.server --no-extensions -e ./my-extension.ts
```
...
In `fullscreen` mode, the transcript scrolls inside the terminal viewport while queued messages, working status, extension widgets, editor, and footer remain fixed at the bottom. Mouse/trackpad input scrolls the region under the pointer; keyboard viewport actions always remain available. Inline images work in terminals that support the Kitty graphics protocol, including Kitty and Ghostty. In iTerm2 they render as text placeholders because its inline-image protocol cannot delete or crop placements during application-owned scrolling. In `regular` mode, midnight.server uses the main screen and terminal-owned scrollback, and iTerm2 inline images continue to render normally. See [Terminal setup](terminal-setup.md) for terminal-specific settings and workarounds.
