> midnight.server can create themes. Ask it to build one for your setup.
...
midnight.server loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.midnight.server/agent/themes/*.json`
- Project: `.midnight.server/themes/*.json` (only after the project is trusted)
...
On first run, midnight.server detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.midnight.server/agent/themes
vim ~/.midnight.server/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, midnight.server reloads it automatically for immediate visual feedback.
...
midnight.server uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, midnight.server falls back to the nearest approximation.
