...
describe("NodeExecutionEnv", () => {
	...
	it.each([
		...
		["an empty override shadows the base value", { MIDNIGHT_SERVER_SESSION_FILE: "" }, "x:"],
		[
			"a string override replaces the base value",
			{ MIDNIGHT_SERVER_SESSION_FILE: "/sessions/current.jsonl" },
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					MIDNIGHT_SERVER_SESSION_FILE: "/stale/parent.jsonl",
					MIDNIGHT_SERVER_CODING_AGENT: "true",
					PI_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const collected = await collectShellOutput(
				env,
				`printf '%s:%s|%s|%s' "\${MIDNIGHT_SERVER_SESSION_FILE+x}" "\${MIDNIGHT_SERVER_SESSION_FILE-}" "$MIDNIGHT_SERVER_CODING_AGENT" "$PI_NODE_ENV_PRESERVED_TEST"`,
				{ env: overrides },
				BACKGROUND_CONTEXT,
			);
